...
#define VIPER_HAS_NODEMCU_MODULE 0
...
static const char *read_native_arg(lua_State *L, int idx, ValueType type, bool unsafe_ptr, ViperNativeArg &out)
{
    ...
    if (is_ptr_type(type))
    {
        ...
        if (unsafe_ptr && lua_isnumber(L, idx))
        {
...
static int l_viper_compile_c(lua_State *L)
{
    ...
    if (setjmp(alloc_scope.env) != 0)
    {
        ...
        destroy_compiled_function(fn);
        return luaL_error(L, "%s", err.c_str("viper.compile_c: out of memory"));
    }
    leave_viper_alloc_scope(alloc_scope);
    ...
    void *ud = lua_newuserdata(L, sizeof(CompiledFunction *));
...
static int l_viper_buf(lua_State *L)
{
    ...
    ViperBuffer *buf = (ViperBuffer *)lua_newuserdata(L, sizeof(ViperBuffer));
...
extern "C" int viper_nodemcu_compile_c(lua_State *L)
{
    return lua_nodemcu_viper::internal::l_viper_compile_c(L);
}

extern "C" int viper_nodemcu_buf(lua_State *L)
{
    return lua_nodemcu_viper::internal::l_viper_buf(L);
}

extern "C" int viper_nodemcu_init(lua_State *L)
{
    lua_nodemcu_viper::internal::create_function_meta(L);
    lua_nodemcu_viper::internal::create_buffer_meta(L);
    return 0;
}

#if VIPER_HAS_NODEMCU_MODULE
static int viper_compile_c(lua_State *L)
{
    return lua_nodemcu_viper::internal::l_viper_compile_c(L);
}

static int viper_buf(lua_State *L)
{
    return lua_nodemcu_viper::internal::l_viper_buf(L);
}

static int viper_module_init(lua_State *L)
{
    lua_nodemcu_viper::internal::create_function_meta(L);
    lua_nodemcu_viper::internal::create_buffer_meta(L);
    return 0;
...
LROT_BEGIN(module, NULL, 0)
    LROT_FUNCENTRY(compile_c, viper_compile_c)
    LROT_FUNCENTRY(buf, viper_buf)
LROT_END(module, NULL, 0)

NODEMCU_MODULE(VIPER, "viper", module, viper_module_init);
#endif

extern "C" int luaopen_viper(lua_State *L)
{
    lua_nodemcu_viper::internal::create_function_meta(L);
    lua_nodemcu_viper::internal::create_buffer_meta(L);

    lua_newtable(L);
    lua_pushcfunction(L, lua_nodemcu_viper::internal::l_viper_compile_c);
    lua_setfield(L, -2, "compile_c");
    lua_pushcfunction(L, lua_nodemcu_viper::internal::l_viper_buf);
    lua_setfield(L, -2, "buf");
    return 1;
}

extern "C" int luaopen_viper_nodemcu(lua_State *L)
{
    return luaopen_viper(L);
}
