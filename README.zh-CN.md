...
Viper 是一个实验性的 Lua 模块，可以在运行时把一个小型 C-like 函数编译成 Xtensa 原生机器码。它主要面向 ESP32 系列芯片上的热点整数循环，尤其是 ESP32-S3 + ESP-IDF / NodeMCU。
...
| ESP32-S3 + Arduino | 可以编译，但通常不能修改必要的 `sdkconfig`，不预期能正常运行 native code |
| ESP32 + ESP-IDF | 理论上可能可用，尚未验证 |
| ESP32 + Arduino | 可以编译，但通常不能修改必要的 `sdkconfig`，不预期能正常运行 native code |
| ESP8266 | 暂不支持 |
...
Arduino 适配层目前更适合做集成实验。很多 Arduino 环境不能方便地修改 Viper 运行所需的 ESP-IDF `sdkconfig`，所以“能编译”不等于“能运行 native code”。实际使用请优先选择 ESP-IDF 或 NodeMCU。
